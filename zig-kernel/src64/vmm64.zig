...
pub var diag_watch: [2]struct { va: u64, len: u64 } = .{
    // CDD №15 p5: краш-VA run14. B-окно = ВЕСЬ slab [0x2000_2407_2000,
    // 0x2000_2407_A000) — видим каждую материализацию/прот-цикл КАЖДОЙ
    // страницы слаба (заголовок чанка 0x24079AC0 = внутри). A-страница —
    // живой сосед (контроль). Арена: t8 mmap(128MB, PROT_NONE, NORESERVE)
    // → align 64MB → munmap краёв → mprotect-коммиты RW (LLVM slab).
    .{ .va = 0x2000_2400_3000, .len = 0x1000 },
    .{ .va = 0x2000_2407_2000, .len = 0x8000 },
};
