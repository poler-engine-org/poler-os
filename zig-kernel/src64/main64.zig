...
fn drmBootInit() void {
    ...
    // (3) BGA I/O (VirtualBox VBoxVGA / Bochs / QEMU -vga std)
    ...
    if (bga_id >= 0xB0C0 and bga_id <= 0xB0C6) {
        ...
        hal.outw(VBE_DISPI_IOPORT_INDEX, 4); // ENABLE | LFB
        hal.outw(VBE_DISPI_IOPORT_DATA, 0x41);

        var vga_bar0: u64 = if (framebuffer.is_available()) framebuffer.getAddr() else 0xE0000000;
        var pci_bus: u16 = 0;
        ...
        puts("[BGA] Hardware rasterizer locked to 1024x768x32 LFB mode (VRAM Phys @0x");
        putHex(vga_bar0);
        puts(")\n");
